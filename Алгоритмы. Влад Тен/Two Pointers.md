...
![[Pasted image 20251117163738.png]]
Вот как здесь всё выглядит на практике. 
**Моя ошибка:** я думал что мы меняем индексами местами то есть nullP = i. Из за этого когда у нас идут два нуля подряд решить невозможно. Нужно просто увеличивать индекс на единицу.
...
```go
func merge(nums1 []int, m int, nums2 []int, n int) {
	p1, p2 := m - 1, n -1 //0, 0, 1, 3. n2 = 2, 4 
	result := m + n - 1

	for p1 >= 0 && p2 >= 0 {
		if nums1[p1] > nums2[p2] { //3 < 4, 3 > 2, 1 < 2
			nums1[result] = nums1[p1]//-, nums[2] == 3, p1 = 0,
			p1-- //1. -, 2.nums1 = [3,4], nums1 = [2,3,4]
		} else {
			nums1[result] = nums2[p2] //nums[3] == 4, p2 = 0, nums[1] = 2, p--
			//выход из цикла
			p2--
		}
		result--
...
```
более интересный кейс этой задачи. Здесь мы начинаем с конца. И добавляем наоборот наибольший элемент в конец. В конце у нас получится итак итак что в nums[1] всё заполнено так что если nums[1] закончился раньше то пробегаемся по nums2 и заполняем именно его. И я кстати неправильный пример выбрал для объяснения самому себе т.к. нули всегда будут в конце. **XD**
...
func backspaceCompare(s string, t string) bool {
	...
	for n1 >= 0 || n2 >= 0 {
		...
		//+ здесь нужна ещё одна проверка чтобы в конечном счёте
		//если бэкспейсов слишком много мы не вышли 
		//за пределы цикла. то есть они классно работают в случае
		//с s = "a###", t = "b###"
			return false
		}
		if (n1 >= 0) != (n2 >= 0) {
